      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div style="display: none;">
          ...
          <template x-if="form.image && (form.image instanceof File || typeof form.image === 'string')">
            <div class="mt-2">
              <img :src="form.image instanceof File ? URL.createObjectURL(form.image) : '/uploads/articles/' + form.image" alt="Preview" class="h-32 object-cover rounded">
        ...
        tinymce.init({
          ...
          setup: function (editor) {
            editor.on('init', function () {
              editor.setContent(Alpine.store('articleForm').form.description || '');
            });
            editor.on('change', function () {
              // Clear error when content changes
              const alpineData = Alpine.store('articleForm');
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('articleForm', {
      form: {
        ...
        slug: <?= json_encode($article['slug'] ?? '') ?>,
        summary: <?= json_encode($article['summary'] ?? '') ?>,
        description: <?= json_encode($article['description'] ?? '') ?>,
        image: <?= json_encode($article['image'] ?? '') ?>,
        status: <?= json_encode($article['status'] ?? 'active') ?>,
        tags: <?= json_encode($article['tags'] ?? []) ?>,
        serial: <?= json_encode($article['serial'] ?? '') ?>,
        createdAt: <?= json_encode($article['created_at'] ?? '') ?>,
        updatedAt: <?= json_encode($article['updated_at'] ?? '') ?>,
      }
    });
  });
    ...
    return {
      ...
      async submitForm() {
        ...
        try {
          ...
          const response = await fetch('<?= site_url('dashboard/article/update') ?>', {
            method: 'POST',
            headers: {
      ...
      handleSuccess(data) {
        ...
        // Update form data
        if (data.article) {
          this.form.createdAt = data.article.created_at?.split(' ')[0] || this.form.createdAt;
          this.form.updatedAt = data.article.updated_at?.split(' ')[0] || this.form.updatedAt;
        }
