...
<div x-data="expenseManager()" x-init="init()" class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8" x-cloak>
    <header class="mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <!-- Empty State -->
        <div x-show="!loading && expenses.length === 0" class="p-4 text-center bg-yellow-50 text-yellow-800">
            No expense records found. Try adjusting your filters or create a new expense record.
        </div>

        <!-- Table Content -->
        <div x-show="!loading && expenses.length > 0" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="table-container">
                    ...
                    <template x-for="(expense, index) in expenses" :key="expense.id">
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900" x-text="expense.formatted_date"></div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900" x-text="expense.wallet_name"></div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="text-sm text-gray-900 line-clamp-2" x-text="expense.description"></div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <div class="text-sm font-medium text-red-600" x-text="`${expense.currency}${new Intl.NumberFormat().format(expense.amount)}`">
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <div class="flex justify-end space-x-2">
                                    <button @click="openEditModal(expense)" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                        </svg>
                                        Edit
                                    </button>
                                    <button @click="openDeleteModal(expense)" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-rose-600 hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500 transition-all duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
        ...
        <!-- Pagination -->
        <div x-show="!loading && expenses.length > 0" class="flex items-center justify-between px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold text-gray-800" x-text="isEditing ? 'Edit Expense' : 'New Expense'"></h2>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end space-x-3">
                ...
                <button @click="submitForm()" class="px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700" x-text="isEditing ? 'Update Expense' : 'Create Expense'">
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" x-cloak>
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold text-gray-800">Delete Expense</h2>
            ...
            <div class="px-6 py-5">
                <div class="flex items-start">
                    ...
                    <div>
                        <p class="text-gray-700">You're about to delete <strong class="font-medium" x-text="selectedExpense.description"></strong>.</p>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end space-x-3">
                ...
                <button @click="deleteExpense()" class="px-4 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
    ...
    function expenseManager() {
        return {
            ...
            fetchExpenses() {
                ...
                let url = `<?= site_url('expense/list') ?>?page=${this.currentPage}&sort=${this.sortField}&direction=${this.sortDirection}`;
                ...
                .then(response => {
                    if (response.success) {
                        this.expenses = response.data.map(expense => ({
                            ...expense,
                            formatted_date: this.formatDate(expense.created_at)
                        }));
                ...
                .catch(error => {
                    ...
                    this.error = error.message || 'Failed to load expense data';
                })
                .finally(() => {
                    this.loading = false;
                });
            ...
            openEditModal(expense) {
                ...
                fetch(`/expense/select/${expense.id}`)
                    .then(response => response.json())
                    .then(expense => {
                        this.form.selectWallet = expense.data.fk_wallet_id;
                        this.form.currency = expense.data.currency;
                        this.form.amount = expense.data.amount;
                        this.form.date = expense.dateOnly;
                        this.form.description = expense.data.description;
                        this.showModal = true;
                    })
                    .catch(error => {
                        console.error('Failed to fetch expense data', error);
                        alert('Something went wrong while fetching expense data.');
                    });
            },
            ...
            submitForm() {
                const url = this.isEditing
                    ? `<?= site_url('expense/update/') ?>${this.form.id}`
                    : '<?= site_url('expense/create') ?>';
                ...
                .then(response => {
                    ...
                    if (this.isEditing) {
                        const idx = this.expenses.findIndex(i => i.id === this.form.id);
                        if (idx !== -1) {
                            this.expenses[idx].description = this.form.description;
                            this.expenses[idx].amount = this.form.amount;
                        }
                    }
            ...
            deleteExpense() {
                fetch(`<?= site_url('expense/delete/') ?>${this.selectedExpense.id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Content-Type': 'application/x-www-form-urlencoded'
                ...
                .then(data => {
                    if (data.success) {
                        this.expenses = this.expenses.filter(w => w.id !== this.selectedExpense.id);
