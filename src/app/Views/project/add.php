<div x-data="projectManager()" x-init="init()" class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8" x-cloak>
  <header class="mb-6">
    ...
    <nav class="flex mt-2">
      <a href="<?= base_url() ?>dashboard/project" class="text-blue-600 hover:text-blue-800 text-sm">← Back to Projects</a>
    </nav>
  </header>
    ...
    return {
      ...
      async submitForm() {
        try {
          ...
          const response = await fetch('<?= site_url('dashboard/project/create') ?>', {
            method: 'POST',
            headers: {
          ...
          this.successMessage = data.message || 'Project created successfully!';
