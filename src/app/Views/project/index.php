...
<div x-data="projectManager()" x-init="init()" class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8" x-cloak>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <!-- Empty State -->
        <div x-show="!loading && projects.length === 0" class="p-4 text-center bg-yellow-50 text-yellow-800">
            No project records found. Try adjusting your filters or create a new project record.
        </div>
        <!-- Table Content -->
        <div x-show="!loading && projects.length > 0" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="table-container">
                    ...
                    <template x-for="(project, index) in projects" :key="project.id">
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900" x-text="project.serial"></div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900" x-text="project.formatted_date"></div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900 font-medium" x-text="project.title"></div>
                            </td>
                            <td style="display: none" class="px-6 py-4">
                                <div class="flex items-center justify-center">
                                    <img :src="project.image ? `<?= base_url() ?>uploads/projects/thumb/${project.image}` : ''" class="h-10 w-10 rounded-md object-cover" alt="Project image" />
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                ...
                                <template x-if="project.status === 'inactive'">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <div class="flex justify-end space-x-2">
                                    <a :href="`/dashboard/project/select/${project.id}`" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                        </svg>
                                        Edit
                                    </a>
                                    <button @click="openDeleteModal(project)" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-rose-600 hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500 transition-all duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
        ...
        <!-- Pagination -->
        <div x-show="!loading && projects.length > 0" class="flex items-center justify-between px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
    ...
    <div class="fixed bottom-6 right-6 md:bottom-8 md:right-8">
        <a href="<?= base_url() ?>dashboard/project/add" class="flex items-center justify-center p-3 bg-green-600 text-white rounded-full shadow-lg hover:bg-green-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold text-gray-800">Delete Project</h2>
            ...
            <div class="px-6 py-5">
                <div class="flex items-start">
                    ...
                    <div>
                        <p class="text-gray-700">You're about to delete <strong class="font-medium" x-text="selectedProject.title"></strong>.</p>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end space-x-3">
                ...
                <button @click="deleteProject()" class="px-4 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
    ...
    function projectManager() {
        return {
            ...
            fetchProjects() {
                ...
                let url = `<?= site_url('dashboard/project/list') ?>?page=${this.currentPage}&sort=${this.sortField}&direction=${this.sortDirection}`;
                ...
                fetch(url, {
                    ...
                    .then(response => {
                        if (response.success) {
                            this.projects = response.data.map(project => ({
                                ...project,
                                formatted_date: this.formatDate(project.created_at)
                            }));
                    ...
                    .catch(error => {
                        ...
                        this.error = error.message || 'Failed to load project data';
                    })
                    .finally(() => {
                        this.loading = false;
                    });
            ...
            deleteProject() {
                fetch(`<?= site_url('dashboard/project/delete/') ?>${this.selectedProject.id}`, {
                    method: 'DELETE',
                    headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            this.projects = this.projects.filter(w => w.id !== this.selectedProject.id);
