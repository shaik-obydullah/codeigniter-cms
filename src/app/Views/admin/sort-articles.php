<?= view('admin/layout/header', ['pageTitle' => $pageTitle, 'currentPage' => 'articles', 'unreadCount' => $unreadCount, 'user' => $user, 'extraHead' => '<script src="https://cdn.jsdelivr.net/npm/sortablejs@1.15.6/Sortable.min.js"></script>']) ?>
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h2 class="text-lg font-semibold text-white">Drag to reorder articles</h2>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="<?= site_url('/dashboard/articles') ?>" class="flex items-center gap-2 bg-gray-800 text-gray-300 border border-gray-700 px-4 py-2.5 rounded-lg hover:text-white hover:bg-gray-700 transition text-sm">
                ...
                <div id="articleList" class="space-y-2 max-w-3xl">
                    ...
                    <div class="article-item flex items-center gap-4 bg-gray-800 border border-gray-700 rounded-xl px-5 py-4 cursor-grab active:cursor-grabbing hover:border-gray-600 transition" data-id="<?= $article->id ?>">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-white font-medium truncate"><?= esc($article->title) ?></p>
                        </div>
                        <span class="px-2.5 py-1 rounded-full text-xs font-medium <?= $article->status === 'published' ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400' ?>">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        saveBtn.addEventListener('click', function () {
            ...
            fetch('<?= site_url('/dashboard/articles/reorder') ?>', {
                method: 'POST',
                headers: {
